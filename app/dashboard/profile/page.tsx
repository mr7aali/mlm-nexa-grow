  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-6 xl:grid-cols-[0.9fr_1.1fr]">
        <Card className="p-6">
          <div className="flex flex-col items-center text-center">
            <div className="relative">
              <div className="grid h-28 w-28 place-items-center rounded-full bg-gradient-to-br from-gold to-purple-light text-4xl font-black text-black">রা</div>
          ...
          <div className="mt-6 space-y-3 rounded-[18px] border border-white/7 bg-elevated p-4 text-sm">
        ...
        <Card className="p-6">
          ...
          <form className="space-y-4" onSubmit={profileForm.handleSubmit(() => setSaved("প্রোফাইল তথ্য ডেমোভাবে সংরক্ষিত হয়েছে।"))}>
            <label className="block"><span className="mb-2 block text-sm text-muted">পুরো নাম</span><Input {...profileForm.register("fullName")} />{profileForm.formState.errors.fullName ? <span className="text-sm text-red-200">{profileForm.formState.errors.fullName.message}</span> : null}</label>
            <label className="block"><span className="mb-2 block text-sm text-muted">ফোন</span><Input {...profileForm.register("phone")} />{profileForm.formState.errors.phone ? <span className="text-sm text-red-200">{profileForm.formState.errors.phone.message}</span> : null}</label>
            {saved ? <p className="rounded-2xl bg-emerald-400/10 px-4 py-3 text-sm text-emerald-200">{saved}</p> : null}
      ...
      <Card className="border-red-400/20 p-6">
        <h3 className="text-2xl font-bold text-red-100">অ্যাকাউন্ট ডিলিট</h3>
