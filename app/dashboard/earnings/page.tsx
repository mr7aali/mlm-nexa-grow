...
export default function EarningsPage() {
  ...
  async function handleWithdrawalSubmit(values: WithdrawForm) {
    try {
      ...
      setSuccess("উইথড্র অনুরোধ জমা হয়েছে।");
      reset({ amount: "", method: "bKash", account: "" });
    } catch (error) {
      setSuccess(getApiErrorMessage(error, "উইথড্র অনুরোধ ব্যর্থ হয়েছে"));
    }
  }
  ...
  return (
    <div className="space-y-6">
      ...
      {earningsLoading ? <p className="text-sm text-muted">আয়ের ডেটা লোড হচ্ছে...</p> : null}
      ...
      <Card className="p-6">
        ...
        <div className="h-80">
          {mounted && monthlyRows.length ? (
            ...
            <div className="grid h-full place-items-center rounded-[18px] border border-line bg-elevated text-muted">{mounted ? "আয়ের চার্ট ডেটা নেই।" : "চার্ট লোড হচ্ছে..."}</div>
          )}
        </div>
      </Card>
        ...
        <table className="w-full min-w-[720px] text-left text-sm">
          ...
          <tbody>
            {withdrawalRows.length ? withdrawalRows.map((item) => (
              <tr key={item.id} className="border-t border-line"><td className="px-5 py-4">{item.date}</td><td className="px-5 py-4 text-gold-light">{taka(item.amount)}</td><td className="px-5 py-4">{item.method}</td><td className="px-5 py-4 text-muted">{item.status}</td></tr>
            )) : (
              <tr><td className="px-5 py-8 text-center text-muted" colSpan={4}>উইথড্র অনুরোধ নেই।</td></tr>
            )}
          </tbody>
        </table>
      </Card>
