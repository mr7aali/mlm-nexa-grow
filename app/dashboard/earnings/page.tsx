    ...
    <div className="space-y-6">
      ...
      <Card className="overflow-x-auto p-0 scrollbar-soft">
        <table className="w-full min-w-[720px] text-left text-sm">
          ...
          <tbody>{withdrawals.map((item) => <tr key={item.id} className="border-t border-line"><td className="px-5 py-4">{item.date}</td><td className="px-5 py-4 text-gold-light">{taka(item.amount)}</td><td className="px-5 py-4">{item.method}</td><td className="px-5 py-4 text-muted">{item.status}</td></tr>)}</tbody>
        </table>
      </Card>
    </div>
  );
